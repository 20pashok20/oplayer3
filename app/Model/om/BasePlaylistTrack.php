...
/**
 * Base class that represents a row from the 'playlist_track' table.
 *
 *
 *
...
abstract class BasePlaylistTrack extends BaseObject implements Persistent
{
     ...
     * Peer class name
     */
    const PEER = 'Model\\PlaylistTrackPeer';
    ...
    /**
     * The value for the playlistid field.
     * @var        int
     */
    ...
    /**
     * Get the [playlistid] column value.
     *
     * @return int
     */
    ...
    /**
     * Set the value of [playlistid] column.
     *
     * @param int $v new value
     * @return PlaylistTrack The current object (for fluent API support)
     */
    public function setPlaylistid($v)
    {
        ...
        if ($this->playlistid !== $v) {
            $this->playlistid = $v;
            $this->modifiedColumns[] = PlaylistTrackPeer::PLAYLISTID;
        }
    ...
    public function setTrack($v)
    {
        ...
        if ($this->track !== $v) {
            $this->track = $v;
            $this->modifiedColumns[] = PlaylistTrackPeer::TRACK;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            $this->playlistid = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
            $this->track = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
            $this->resetModified();
            ...
            return $startcol + 3; // 3 = PlaylistTrackPeer::NUM_HYDRATE_COLUMNS.
    ...
    public function ensureConsistency()
    {
        ...
        if ($this->aPlaylist !== null && $this->playlistid !== $this->aPlaylist->getId()) {
            $this->aPlaylist = null;
        }
    } // ensureConsistency
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(PlaylistTrackPeer::PLAYLISTID)) {
            $modifiedColumns[':p' . $index++]  = '`playlistId`';
        }
        if ($this->isColumnModified(PlaylistTrackPeer::TRACK)) {
            $modifiedColumns[':p' . $index++]  = '`track`';
        }

        $sql = sprintf(
            'INSERT INTO `playlist_track` (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case '`id`':
                        ...
                        break;
                    case '`playlistId`':
                        $stmt->bindValue($identifier, $this->playlistid, PDO::PARAM_INT);
                        break;
                    case '`track`':
    ...
    public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) $this->setTrack($arr[$keys[2]]);
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(PlaylistTrackPeer::PLAYLISTID)) $criteria->add(PlaylistTrackPeer::PLAYLISTID, $this->playlistid);
        if ($this->isColumnModified(PlaylistTrackPeer::TRACK)) $criteria->add(PlaylistTrackPeer::TRACK, $this->track);
    ...
    {
        if ($this->aPlaylist === null && ($this->playlistid !== null) && $doQuery) {
            $this->aPlaylist = PlaylistQuery::create()->findPk($this->playlistid, $con);
